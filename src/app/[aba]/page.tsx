  ...
  const filteredRelatos = useMemo(() => {
    return relatos.filter(relato => {
      const matchesSearch =
        ...
        (relato.tipo_relato?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
        (relato.ambiente?.toLowerCase().includes(searchTerm.toLowerCase()) ?? false) ||
                ...
                <TableBody>
                  {filteredRelatos.map((relato, index) => (
                    <TableRow key={`${relato.numero_demanda}-${relato.numero_relato}-${index}`}>
                      ...
                      <TableCell>{relato.tipo_relato || "-"}</TableCell>
                      <TableCell>{relato.ambiente || "-"}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          <Link href={`/${aba}/${relato.numero_relato}`}>
                            <Button variant="ghost" size="sm">
                              <Eye className="h-4 w-4" />
                            </Button>
                          </Link>
                          <Link
                            href={`/${aba}/${relato.numero_relato}/editar`}
                          >
                            <Button variant="ghost" size="sm">
