...
import { FileText, CheckCircle2, Clock, XCircle, Users } from 'lucide-react';
...
export function KPICards() {
  ...
  const emAndamento = relatos.filter((r) => r.status === StatusEnum.EM_ANDAMENTO).length;
  const concluidos = relatos.filter((r) => r.status === StatusEnum.CONCLUIDA).length;
  const cancelados = relatos.filter((r) => r.status === StatusEnum.CANCELADA).length;
      ...
      {kpis.map((kpi) => (
        <Card key={kpi.title}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <kpi.icon className={`h-4 w-4 ${kpi.className || 'text-gray-500'}`} />
          </CardHeader>
          <CardContent>
